...
int main() {
    ...
    // listen() -> 서버 소켓을 passive socket 으로 변경. 이제 연결을 받을 차례
    ...
    while (true) {
        ...
        // 만일 struct timeval 형태의 변수를 만들어 timeout 시간을 0으로 하면,
        // 기다리는 시간이 0, 즉 바로 확인만 하고 반환함
        int numReady = select(maxFd + 1, &rset, NULL, NULL, NULL);
        if (numReady < 0){
            cerr << "select() failed: " << strerror(errno) << endl;
            continue;
        } else if (numReady == 0) {
            // 어떤 소켓에도 데이터가 들어오지 않고 있음
            continue;
        }
        ...
        if (FD_ISSET(serverSock, &rset)){
            // 새로 들어오는 연결을 받음
            // accept는 이 연결을 위한 새로운 active socket 을 생성함
            memset(&sin, 0, sizeof(sin));
        ...
        for (int clientSock : clientSocks) {
            ...
            if (numRecv == 0){
                ...
                // 읽은 데이터를 그대로 돌려줌
                // send() 는 요청한 byte 수만큼 전송 보장하지 않으므로 반복해서 send를 호출해야 될 수 있음
                int offset = 0;
        ...
        // 닫아야 하는 소켓 정리
        // 위 For 문 안에서 처리 하지 않는 이유?
        // ClientSocks.erase() 가 for 도는 중간에 clientSocks 내용 변경하기 때문임
